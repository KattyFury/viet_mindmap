...
- **Spacing giữa sibling = theo PROFILE TỪNG LEVEL, không phải cả subtree gộp 1 khoảng (sửa 2026-09-11, xem `HANDOFF.md`).** Trước đây dùng `subtreeBounds()` (min/max Y của TOÀN subtree) để tính khoảng cách + canh giữa cụm — bug: 1 sibling có nhiều cháu (VD 8 con) thì cháu nó tự xoè đối xứng quanh nó, khiến sibling đó cần rất nhiều "chỗ trống phía trên/dưới chính nó", và bước canh-giữa-cả-cụm-theo-parent kéo sibling bên cạnh (không hề có cháu) bay xa lên tít trên, dù 2 đứa không hề chồng lấn thật (khác X band vì cháu ở level sâu hơn luôn dịch thêm sang phải/trái). Fix: `subtreeLevelProfile()` đo before/after theo TỪNG absolute level riêng (vì `branchOffset` chỉ phụ thuộc level+hướng → mọi node cùng level cùng hướng LUÔN cùng 1 dải X, khác level thì không thể chồng nhau bất kể Y). Canh giữa cả CỤM sibling-trực-tiếp (stackMin/stackMax) dùng box RIÊNG của sibling đầu/cuối (không phải mép subtree) → 2 sibling cùng cấp luôn nằm cân đối/đều quanh parent bất kể 1 bên có bao nhiêu cháu; cháu bên trong vẫn tự do xoè đối xứng quanh sibling đó mà không kéo ai khác đi theo.
- **3+ sibling — dùng CONTOUR LUỸ KẾ, không so cặp liền kề (bug tiếp theo, sửa cùng ngày, xem `HANDOFF.md` mục 5b):** ban đầu chỉ so `profiles[i-1]` với `profiles[i]` (cặp liền kề) — bug: nếu sibling giữa RỖNG (không cháu ở 1 level nào đó), nó không "mang" được yêu cầu né nhau giữa 2 sibling 2 bên nó sang bước kế tiếp → 2 sibling có cháu ở 2 đầu chồng lấn lên nhau dù không liền kề tính toán. Fix: `minCenterAgainstContour()` so mỗi sibling mới với **contour = độ vươn xa nhất tuyệt đối của TẤT CẢ sibling đã đặt trước đó, luỹ kế theo từng level** (không chỉ sibling liền trước) — kỹ thuật "running contour" chuẩn của tidy-tree layout (Reingold-Tilford/Walker), áp dụng rời rạc theo level.
